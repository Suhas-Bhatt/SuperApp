...
// ==========================================
// Mock Data Helpers (Fallback when APIs fail)
// ==========================================
...
const getMockMoviesByGenre = (query) => {
  const mockMovies = {
    Action: [
      {
...
const getMockMovieDetails = (imdbID) => ({
 ...
 * Fallbacks to mock data if no API key is set or if request fails.
 * 
...
export const fetchCurrentWeather = async (city) => {
  if (isPlaceholder(WEATHER_API_KEY)) {
    console.log('Using mock weather data (Placeholder key)');
  ...
  try {
    ...
    console.warn('Weather service failed (falling back to mock weather data):', error.message || error);
    return getMockWeatherData(city);
  }
};
 ...
 * Fallbacks to mock headlines if no API key is set or if request fails.
 * 
...
export const fetchTopHeadlines = async (category = 'general') => {
  if (isPlaceholder(NEWS_API_KEY)) {
    console.log('Using mock news data (Placeholder key)');
  ...
  try {
    ...
    console.warn('News service failed (falling back to mock news headlines):', error.message || error);
    return getMockNewsArticles();
  }
};
 ...
 * Fallbacks to mock movie list if no API key is set or if request fails.
 * 
...
export const searchMovieByGenre = async (query) => {
  if (isPlaceholder(MOVIE_API_KEY)) {
    console.log('Using mock movie search data (Placeholder key)');
    return getMockMoviesByGenre(query);
  }
  ...
  try {
    ...
    console.warn('Movie search service failed (falling back to mock movies list):', error.message || error);
    return getMockMoviesByGenre(query);
  }
};
 ...
 * Fallbacks to mock details if no API key is set, if a mock ID is used, or if request fails.
 * 
...
export const fetchMovieDetails = async (imdbID) => {
  if (isPlaceholder(MOVIE_API_KEY) || imdbID.startsWith('tt_mock_')) {
    console.log('Using mock movie details (Placeholder or mock ID)');
    return getMockMovieDetails(imdbID);
  }
  ...
  try {
    ...
    console.warn('Movie details fetch failed (falling back to mock movie detail):', error.message || error);
    return getMockMovieDetails(imdbID);
  }
};
